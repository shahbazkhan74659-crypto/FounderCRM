...
- Consequences: `server/next.config.ts`'s dev-only rewrite and `frontend/vite.config.ts`'s HMR pinning are temporary scaffolding for local development — revisit both when production single-port serving is actually built (see `PHASES.md`'s Phase 24, or wherever the owner schedules it). `env` loading for `DATABASE_URL` must happen inside route-handler-reachable code (e.g. `server/lib/db.ts`) rather than in `next.config.ts` — Turbopack dev route handlers don't inherit `process.env` mutations made at config-load time in a separate context. **Correction (2026-09-14, found during Phase 5):** the original fix used `@next/env`'s `loadEnvConfig`, which turned out to be unreliable across Turbopack recompiles — it internally short-circuits (returns without merging any values) once Next's own automatic env loading has already set an internal `__NEXT_PROCESSED_ENV` flag on `process.env`, which happens nondeterministically relative to our own call depending on compile order. This caused `DATABASE_URL` to silently go missing after adding new routes (working right after a fresh server start, then breaking after Turbopack recompiled). Fixed by switching `server/lib/db.ts` to plain `dotenv`'s `config({ path })` instead, which has no such cross-check and reliably merges the root `.env` into `process.env` every time. Any other code needing the root `.env` (e.g. standalone scripts) should use plain `dotenv`, not `@next/env`.
...
## Decision: Phase 5 login backend mirrors RajuApp's actual implementation, with hashed session tokens

- Status: Accepted
- Date: 2026-09-14
- Context: `DECISIONS.md`'s auth-approach entry said to follow "RajuApp's pattern" without specifying exact parameters. Before implementing, researched RajuApp's actual current code (`C:\RajuApp`, a sibling Next.js/TypeScript app using raw `pg`) rather than relying on paraphrase.
- Decision: Mirror RajuApp's pattern exactly except for one deliberate change:
  - **Password hashing**: `bcryptjs`, cost factor 10 (`bcrypt.hash(password, 10)` / `bcrypt.compare`) — identical to RajuApp.
  - **Session token**: `crypto.randomBytes(32).toString("hex")` (64 hex chars), 30-day TTL — identical to RajuApp.
  - **Deviation**: RajuApp stores the raw token in plaintext as `sessions.id` (the primary key). FounderCRM instead stores `sha256(token)` as `sessions.token_hash` (the primary key) — the raw token only ever exists in the client's cookie, never at rest in the DB. Owner's explicit choice when asked, over mirroring RajuApp exactly.
  - **Cookie**: `foundercrm_session` (RajuApp: `trackme_session` — project-specific), `httpOnly`, `sameSite: "lax"`, `secure` in production, `path: "/"`, `expires` set to the session's `expires_at`. Owner's explicit choice to set the cookie directly from the login response (not return the token in the JSON body) even though no frontend consumes it yet (Phase 7's job).
  - **Not mirrored**: RajuApp's in-memory per-IP login rate limiter — outside this phase's locked scope.
  - **Migration tooling**: unlike RajuApp's single hand-maintained `schema.sql`, FounderCRM uses its own already-chosen `node-pg-migrate` (Phase 1b) — `server/migrations/1789376319433_create-users-and-sessions.js`.
  - **Failed-login response**: one generic `401 {"error":"Invalid username or password"}` for both an unknown username and a wrong password — owner's explicit choice, standard practice to avoid leaking which usernames exist.
- Reasoning: Faithful to the "follow RajuApp's pattern" decision where it matters (hashing cost, token format/length, session TTL, schema shape) while closing a real security gap the owner flagged when asked (plaintext token storage) — a deliberate, informed departure, not a guess.
- Consequences: `server/lib/auth.ts` holds the shared `verifyPassword`/`createSession` helpers; any future phase adding session-validation middleware (Phase 21) must hash the incoming cookie token with SHA-256 before looking it up by `sessions.token_hash`, not query by the raw token directly.
